...
contract ProfessionalTokenLaunchpad is Ownable, ReentrancyGuard, Pausable, AccessControl {
    ...
    function createTokenWithLiquidity(
        ...
        require(creatorTokenCount[msg.sender] < maxTokensPerCreator, "Max tokens per creator exceeded");
        
        // Check creation fee + liquidity BNB
        uint256 totalRequired = tokenCreationFee + liquidityBNB;
        require(msg.value >= totalRequired, "Insufficient BNB provided");
        
        // Validate parameters
        require(bytes(name).length > 0 && bytes(name).length <= 50, "Token name must be 1-50 characters");
        require(bytes(symbol).length >= 3 && bytes(symbol).length <= 10, "Token symbol must be 3-10 characters");
        require(totalSupply > 0, "Invalid total supply");
        require(tokenStartTime >= block.timestamp, "Invalid start time");
        require(liquidityBNB >= minimumLiquidityBNB && liquidityBNB <= maximumLiquidityBNB, "Liquidity BNB out of range");
        ...
        for (uint i = 0; i < symbolBytes.length; i++) {
            require(
                (symbolBytes[i] >= 0x41 && symbolBytes[i] <= 0x5A) || // A-Z
                (symbolBytes[i] >= 0x61 && symbolBytes[i] <= 0x7A) || // a-z
                (symbolBytes[i] >= 0x30 && symbolBytes[i] <= 0x39),   // 0-9
                "Symbol must contain only alphanumeric characters"
            );
        }
        
        // Calculate liquidity amounts
        uint256 liquidityTokenAmount = (totalSupply * LIQUIDITY_PERCENTAGE) / LIQUIDITY_DENOMINATOR;
        
        // Create new professional token
        ProfessionalMemeToken newToken = new ProfessionalMemeToken(
            name,
            symbol,
            totalSupply,
            msg.sender, // creator
        ...
        // Set PancakeSwap addresses
        newToken.setPancakeAddresses(
            address(pancakeRouter),
            address(pancakeFactory),
            address(0) // Will be set after pair creation
        );
        
        // Create pair first (if it doesn't exist)
        address pairAddress = pancakeFactory.getPair(tokenAddress, WBNB);
        if (pairAddress == address(0)) {
            pairAddress = pancakeFactory.createPair(tokenAddress, WBNB);
        }
        require(pairAddress != address(0), "Pair creation failed");
        
        // Set the pair in the token contract
        newToken.setPancakeAddresses(
            address(pancakeRouter),
            address(pancakeFactory),
            pairAddress
        ...
        require(contractBalance >= liquidityTokenAmount, "Insufficient token balance for liquidity");
        
        // Approve router to spend tokens
        newToken.approve(address(pancakeRouter), 0);
        newToken.approve(address(pancakeRouter), liquidityTokenAmount);
        
        // Verify approval
        uint256 allowance = newToken.allowance(address(this), address(pancakeRouter));
        require(allowance >= liquidityTokenAmount, "Token approval failed");
        
        // Add liquidity to PancakeSwap
        (uint256 tokenAmount, uint256 bnbAmount, uint256 liquidity) = pancakeRouter.addLiquidityETH{value: liquidityBNB}(
            tokenAddress,
            liquidityTokenAmount,
            (liquidityTokenAmount * 95) / 100, // 5% slippage tolerance
            (liquidityBNB * 95) / 100, // 5% slippage tolerance
            address(this), // LP tokens go to this contract (locked)
            block.timestamp + 300 // 5 minute deadline
        );
        
        require(liquidity > 0, "Liquidity addition failed");
        
        // Store LP token balance and lock time
        tokenLPBalance[tokenAddress] = liquidity;
        tokenLockTime[tokenAddress] = block.timestamp + 365 days; // 1 year lock
        
        // Calculate launch price
        uint256 launchPrice = (bnbAmount * 1e18) / tokenAmount;
        ...
        tokens[tokenAddress] = TokenInfo({
            ...
            marketCap: bnbAmount * 2, // Initial market cap estimate
            volume24h: 0,
            holders: 1,
            category: category,
            verified: false,
        ...
        categoryTokens[category].push(tokenAddress);
        allTokens.push(tokenAddress);
        
        // Update creator token count
        creatorTokenCount[msg.sender]++;
        
        // Update statistics
        totalTokensCreated++;
        totalCreationFeesCollected += tokenCreationFee;
        totalLiquidityProvided += bnbAmount;
        
        // Transfer creation fee to platform
        if (tokenCreationFee > 0) {
            (bool feeSuccess, ) = payable(platformFeeRecipient).call{value: tokenCreationFee}("");
            require(feeSuccess, "Fee transfer failed");
        }
        
        // Refund excess BNB
        uint256 bnbUsed = tokenCreationFee + bnbAmount;
        uint256 excessBNB = msg.value - bnbUsed;
        if (excessBNB > 0) {
            (bool refundSuccess, ) = payable(msg.sender).call{value: excessBNB}("");
        ...
        emit LiquidityAdded(tokenAddress, pairAddress, tokenAmount, bnbAmount, liquidity);
    ...
    function enableTrading(address tokenAddress) external {
        ...
        require(!tokenInfo.tradingEnabled, "Trading already enabled");
        
        ProfessionalMemeToken token = ProfessionalMemeToken(tokenAddress);
        require(block.timestamp >= token.tokenStartTime(), "Token start time not reached");
        
        // Enable trading on the token contract
        token.setTradingEnabled(true);
        tokenInfo.tradingEnabled = true;
        
        emit TradingEnabled(tokenAddress, tokenInfo.pairAddress);
    }
